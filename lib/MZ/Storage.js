...
MZ.Storage = function() {
  ...
  this.db.transaction(function (tx) {
    tx.executeSql('CREATE TABLE IF NOT EXISTS map (id unique, fieldx, fieldy, type)');
  });
}
...
MZ.Storage.prototype.setMapField = function(id, fieldX, fieldY, type) {
  var time = nx.util.time();
  this.db.transaction(function (tx) {
    tx.executeSql('INSERT INTO map (id, fieldx, fieldy, type) VALUES (?, ?, ?, ?)', [id, fieldX, fieldY, type]);
  });
  this.db.transaction(function (tx) {
    tx.executeSql('UPDATE map SET type=? WHERE id=?', [type, id]);
  });
}
