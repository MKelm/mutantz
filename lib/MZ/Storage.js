...
MZ.Storage = function() {
  ...
  this.db.transaction(function (tx) {
    tx.executeSql('CREATE TABLE IF NOT EXISTS map (id unique, fieldx, fieldy, type, roomType)');
  });
  this.db.transaction(function (tx) {
...
MZ.Storage.prototype.setMapField = function(id, fieldX, fieldY, type, roomType) {
  var scope = this;
  this.db.transaction(function (tx) {
    tx.executeSql(
      'INSERT INTO map (id, fieldx, fieldy, type, roomType) VALUES (?, ?, ?, ?, ?)',
      [id, fieldX, fieldY, type, roomType],
      function() {},
      function() {
        scope.db.transaction(function (tx) {
          tx.executeSql('UPDATE map SET type=?, roomType=? WHERE id=?', [type, roomType, id],
