...
MZ.Game.prototype.hidePanels = function() {
  ...
  this.level.grid.selectedFieldTypeId = -1;

  this.displayAreasPanel.hide();
  this.level.grid.selectedAreaTypeId = -1;

  this.displayEnemiesPanel.hide();
  this.level.enemies.selectedEnemyTypeId = -1;
}
...
MZ.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "panel-area-click":
      scope.level.grid.selectedAreaTypeId = event.content.areaTypeId;
      break;
    case "panel-enemy-click":
      scope.level.enemies.selectedEnemyTypeId = event.content.enemyTypeId;
      break;
  }
}