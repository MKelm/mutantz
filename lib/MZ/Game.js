...
MZ.Game.prototype.handleEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "display-map-mousedown":
      // movement logic
      scope.display.mouseMoveCoor = { x: 0, y: 0 };
      ...
      event.content.mouse.target.mouseup = function() {
        scope.dispatchEvent( { type: 'display-map-mouseup', content: event.content } );
      };
      scope.addEventListener('display-map-mouseup', mz.util.getEventListener(scope, "handleEvent"));
      // field selection
      scope.display.mapFieldSelection(scope.getMapFieldPositionByMouse(event.content.mouse), true);
      break;
    case "display-map-mousemove":
      // movement logic
      if (scope.display.mouseMoveCoor.x > 0 && scope.display.mouseMoveCoor.y > 0) {
        var diff = {
          x: event.content.mouse.originalEvent.screenX - scope.display.mouseMoveCoor.x,
          y: event.content.mouse.originalEvent.screenY - scope.display.mouseMoveCoor.y
        };
        scope.display.moveMap(diff);
      }
      scope.display.mouseMoveCoor = {
        x: event.content.mouse.originalEvent.screenX,
        y: event.content.mouse.originalEvent.screenY
      };
      break;
    case "display-map-mouseup":
      ...
      event.content.mouse.target.mouseup = null;
      scope.removeEventListener('display-map-mouseup', mz.util.getEventListener(scope, "handleEvent"));
      // field deselection
      scope.display.mapFieldSelection(scope.getMapFieldPositionByMouse(event.content.mouse), false);
      break;
  }
}

MZ.Game.prototype.getMapFieldPositionByMouse = function(mouse) {
  // field detection
  ...
  var fieldX = this.map.fieldAmount.x / 2 +
               Math.floor(clickPosX / this.display.mapContainer.scale.x / this.map.fieldSize.width),
      fieldY = this.map.fieldAmount.y / 2 +
               Math.floor(clickPosY / this.display.mapContainer.scale.x / this.map.fieldSize.height);
