...
MZ.Game = function() {
  ...
  var id = this.objects.setObject(1, {x: 12, y: 12}, null);
  ...
  this.displayStaffPanel = new MZ.DisplayPanelStaff();

  // register interaction event listeners
  this.addEventListener('object-add-to-field', mz.util.getEventListener(this, "handleObjectEvent"));
  this.addEventListener('object-field-construction-task', mz.util.getEventListener(this, "handleObjectEvent"));
  this.addEventListener('object-movement-complete', mz.util.getEventListener(this, "handleObjectEvent"));
  ...
  this.addEventListener('panel-staff-click', mz.util.getEventListener(this, "handlePanelEvent"));
...
MZ.Game.prototype.hidePanels = function() {
  this.displayRoomsPanel.hide();
  this.displayFieldsPanel.hide();
  this.displayStaffPanel.hide();
  this.display.selectedFieldTypeId = -1;
  this.display.selectedRoomTypeId = -1;
  this.display.selectedStaffTypeId = -1;
}

MZ.Game.prototype.handleObjectEvent = function(scope, event) {
  switch (event.type) {
    case "object-add-to-field":
      var id = scope.objects.setObject(event.content.typeId, event.content.fieldPosition, null);
      scope.display.drawObject(scope.objects.list[id]);
      break;
    case "object-field-construction-task":
      // transport ship task, if a field has been set as construction
      this.transportShip.tasks.setTask(
        "field-construction", { x: event.content.fieldPosition.x, y: event.content.fieldPosition.y }
      );
      break;
    case "object-movement-complete":
      if (event.content.task.handle == "field-construction") {
        scope.map.setFieldToReady(
          event.content.task.targetPosition.x, event.content.task.targetPosition.y
        );
      }
      var status = event.content.object.tasks.setNextTask();
      if (status) {
        scope.display.moveObject(event.content.object);
      }
      break;
  }
}
...
MZ.Game.prototype.handlePanelEvent = function(scope, event) {
  switch (event.type) {
    ...
    case "panel-room-click":
      ...
      break;
    case "panel-staff-click":
      scope.display.selectedStaffTypeId = event.content.staffTypeId;
      break;
  }
}
