...
MZ.Display = function() {
  ...
  this.lastSelectedMapField = {x: null, y: null};
...
MZ.Display.prototype.mapFieldSelection = function(position, deselect) {
  if (typeof position != "undefined") {

    if (!deselect && (position.x != this.lastSelectedMapField.x || position.y != this.lastSelectedMapField.y)) {
      var sprite = new PIXI.Sprite(PIXI.Texture.fromFrame("data/gfx/field_selection.png"));
      sprite.position = {
        x: -1 * mz.game.map.fieldAmount.x * mz.game.map.fieldSize.width / 2 + position.x * mz.game.map.fieldSize.width,
        y: -1 * mz.game.map.fieldAmount.y * mz.game.map.fieldSize.height / 2 + position.y * mz.game.map.fieldSize.height
      };
      this.mapContainer.addChild(sprite);
      this.lastSelectedMapField = position;
      if (typeof this.selectedMapFields[position.y] == "undefined") this.selectedMapFields[position.y] = [];
      this.selectedMapFields[position.y][position.x] = sprite;

    } else if (deselect == true) {
      for (var y in this.selectedMapFields) {
        for (var x in this.selectedMapFields[y]) {
          if (this.mapContainer.children.indexOf(this.selectedMapFields[y][x]) !== -1)
            this.mapContainer.removeChild(this.selectedMapFields[y][x]);
          this.selectedMapFields[y][x] = null;
        }
      }
    }
  }
