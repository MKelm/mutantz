  ...
  for (var fY = 0; fY < mz.game.map.fieldAmount.y; fY++) {
    ...
    for (var fX = 0; fX < mz.game.map.fieldAmount.x; fX++) {
      if (mz.game.map.loadedFields[fY][fX].visible == false) {
        ...
        var selectionSprite = new PIXI.Sprite(PIXI.Texture.fromFrame("data/gfx/blank.png"));
        selectionSprite.position = { x: x, y: y };
        this.mapContainer.addChild(selectionSprite);

        mz.game.map.loadedFields[fY][fX].visible = true;
        mz.game.map.loadedFields[fY][fX].sprites = { bg: sprite, selection: selectionSprite };
      } else {
        // update map field
      }
...
MZ.Display.prototype.mapFieldSelection = function(position, activate) {
  if (typeof position != "undefined") {
    mz.game.map.loadedFields[position.y][position.x].sprites.selection.texture = PIXI.Texture.fromFrame(
      (activate == true) ? "data/gfx/field_selection.png" : "data/gfx/blank.png"
    );
  }
}