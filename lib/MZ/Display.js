  ...
  for (var fY = 0; fY < mz.game.map.fieldAmount.y; fY++) {
    ...
    for (var fX = 0; fX < mz.game.map.fieldAmount.x; fX++) {
      if (mz.game.map.loadedFields[fY][fX].visible == false) {
        ...
        mz.game.map.loadedFields[fY][fX].sprites = { bg: sprite, selection: {} };
      } else {
        // update map field
      }
...
MZ.Display.prototype.mapFieldSelection = function(position, activate) {
  if (typeof position != "undefined") {
    var fieldSelectionSprite = mz.game.map.loadedFields[position.y][position.x].sprites.selection;
    if (activate == true) {
      var selectionSprite = new PIXI.Sprite(PIXI.Texture.fromFrame("data/gfx/field_selection.png"));
      selectionSprite.position = {
      ...
      this.mapContainer.addChild(selectionSprite);
      mz.game.map.loadedFields[position.y][position.x].sprites.selection = selectionSprite;
    } else if (this.mapContainer.children.indexOf(fieldSelectionSprite) !== -1) {
      this.mapContainer.removeChild(fieldSelectionSprite);
    }
  }
}