    ...
    if (typeof grid.fields[fY] != "undefined") {
      for (var fX = 0; fX < grid.size.width; fX++) {
        if (typeof grid.fields[fY][fX] != "undefined" &&
            this.gridFieldsContainer.children.indexOf(grid.fields[fY][fX].sprite) !== -1)
          this.gridFieldsContainer.removeChild(grid.fields[fY][fX].sprite);
      }
    }
  }
}
...
MZ.Display.prototype.initializeGridInteraction = function() {
  var grid = mz.game.level.grid, startX = x = -1 * grid.size.width / 2 * grid.fieldSize.width,
