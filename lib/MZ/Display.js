...
MZ.Display.prototype.initialize = function() {
  ...
  this.container.addChild(this.gridAreasGfx);
...
MZ.Display.prototype.activateGridZoom = function() {
  ...
  $('html').bind('mousewheel', function(e) {
    if (e.originalEvent.wheelDelta / 120 > 0 && scope.gridContainer.scale.x < 1.9) {
      ...
      scope.gridAreasGfx.scale.x += 0.1;
      scope.gridAreasGfx.scale.y += 0.1;
...
MZ.Display.prototype.moveGrid = function(change) {
  ...
  this.gridAreasGfx.position.x += change.x;
  this.gridAreasGfx.position.y += change.y;
...
MZ.Display.prototype.clearGrid = function() {
  var grid = mz.game.level.grid;
  for (var fY = 0; fY < grid.size.height; fY++) {
    if (typeof grid.fields[fY] != "undefined") {
      for (var fX = 0; fX < grid.size.width; fX++) {
        if (typeof grid.fields[fY][fX] != "undefined" &&
            this.gridContainer.children.indexOf(grid.fields[fY][fX].sprite) !== -1)
          this.gridContainer.removeChild(grid.fields[fY][fX].sprite);
      }
    }
  }
}
...
MZ.Display.prototype.drawGrid = function() {
  ...
  for (var fY = 0; fY < grid.size.height; fY++) {
    ...
    for (var fX = 0; fX < grid.size.width; fX++) {
      ...
      if (grid.fields[fY][fX].areaType > 0) {
