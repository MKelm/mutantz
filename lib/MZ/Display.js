...
MZ.Display.prototype.moveObject = function(object) {
  if (object.movement == false &&
      object.tasks.currentTask !== null && typeof object.tasks.list[object.tasks.currentTask] != "undefined") {
    ...
    for (var i = 0; i < task.path.length; i++) {
      ...
      tweens[i] = function(iI, iLastX, iLastY, iNextX, iNextY, iLastField, iWithRoation) {
        ...
        return new TWEEN.Tween( { x: iLastX, y: iLastY } )
          ...
          .onUpdate( function () {
            if (iWithRoation) object.sprite.rotation = Math.radians(task.path[iI].rotation);
            object.sprite.position.x = this.x;
            object.sprite.position.y = this.y;
          })
          .onComplete( function () {
            object.position.x = task.path[iI].x;
            object.position.y = task.path[iI].y;
            if (iLastField == true) object.movement = false;
          });
