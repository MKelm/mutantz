...
// todo: debug for higher resolutions!!!
...
MZ.Map.prototype.fieldSelection = function(mouse, mode) {
  ...
  switch (mode) {
    ...
    case "move":
      ...
      var posX = Math.min(gfxPos.x, this.fieldSelectionPosition.x);
      var posY = Math.min(gfxPos.y, this.fieldSelectionPosition.y);
      var maxX = Math.max(gfxPos.x, this.fieldSelectionPosition.x);
      var maxY = Math.max(gfxPos.y, this.fieldSelectionPosition.y);

      var diffX = maxX-posX, diffY = maxY-posY;
      mz.game.display.drawSelection(
        mode, { x: posX, y: posY },
        (diffX > 0) ? diffX + 100 : diffX, (diffY > 0) ? diffY + 100 : diffX
      );
      break;
    case "end":
