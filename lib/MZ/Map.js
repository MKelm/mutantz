...
MZ.Map.prototype.loadFields = function(fields) {
  ...
  for (var fY = 0; fY < this.fieldAmount.y; fY++) {
    ...
    for (var fX = 0; fX < this.fieldAmount.x; fX++) {
      this.loadedFields[fY].push({ type: 0, visible: false });
    }
  }

  if (fields.length > 0) {
    for (var i = 0; i < fields.length; i++) {
      var fX = fields[i].fieldx, fY = fields[i].fieldy;
      this.loadedFields[fY][fX].type = fields[i].type;
    }
  }
}