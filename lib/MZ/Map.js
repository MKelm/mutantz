  ...
  switch (mode) {
    ...
    case "end":
      if (mz.game.display.selectedFieldTypeId > -1 || mz.game.display.selectedRoomTypeId > -1
          || mz.game.display.selectedStaffTypeId > -1) {
        var selectedFields = [];
        ...
        for (var fY = minY; fY <= maxY; fY++) {
          for (var fX = minX; fX <= maxX; fX++) {
            ...
            var field = this.loadedFields[fY][fX], fieldChange = false;
            ...
            // set staff object to field position
            if (mz.game.display.selectedStaffTypeId > -1 && this.fieldTypes[field.type].block == false) {
              mz.game.dispatchEvent(
                { type: 'object-add-to-field',
                  content: { fieldPosition: { x: fX, y: fY }, typeId: mz.game.display.selectedStaffTypeId }
                }
              );
            }
            if (fieldChange == true) {
