...
MZ.Map.prototype.calculatePath = function (position, targetPosition, ignoreBlocked) {
  ...
  // calculate rotations
  var checkPath = [[position.x, position.y]];
  for (var i = 0; i < path.length; i++) {
    checkPath.push(path[i]);
  }
  checkPath.push([targetPosition.x, targetPosition.y]);

  for (var i = 0; i < checkPath.length - 2; i++) {
    if (typeof path[i+1] != "undefined") {
      dirX = (path[i][0] > path[i+1][0]) ? -1 : (path[i][0] == path[i+1][0]) ? 0 : 1;
      dirY = (path[i][1] > path[i+1][1]) ? -1 : (path[i][1] == path[i+1][1]) ? 0 : 1;
