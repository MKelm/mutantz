  ...
  for (var fY = 0; fY < this.fieldAmount.y; fY++) {
    ...
    for (var fX = 0; fX < this.fieldAmount.x; fX++) {
      ...
      this.loadedFields[fY].push(
        { id: id, type: type, visible: false, sprite: null, roomType: roomType }
      );
      if (typeof fields[id] == "undefined") mz.storage.setMapField(id, fX, fY, type, -1);
      id++;
    }
  }
...
MZ.Map.prototype.fieldPositionByMouse = function(mouse) {
  ...
  var clickPosX = (oEvent.layerX - mz.pixi.screen.width / mz.pixi.screen.ratio / 2)
                  - target.parent.position.x,
      clickPosY = (oEvent.layerY - mz.pixi.screen.height / mz.pixi.screen.ratio / 2)
                  - target.parent.position.y;
  // map field properties and current map container scale
  var fieldX = this.fieldAmount.x / 2 +
               Math.round(clickPosX / target.parent.scale.x / this.fieldSize.width),
      fieldY = this.fieldAmount.y / 2 +
               Math.round(clickPosY / target.parent.scale.x / this.fieldSize.height);
      ...
      if (mz.game.display.selectedFieldTypeId > -1 || mz.game.display.selectedRoomTypeId > -1) {
        ...
        for (var fY = minY; fY <= maxY; fY++) {
          for (var fX = minX; fX <= maxX; fX++) {
            ...
            console.log(field.roomType);
            mz.storage.setMapField(field.id, null, null, field.type, field.roomType);
          }
        }
        mz.game.display.drawMap();
      }
